<template>
  ...
  <div class="tabs">
    ...
    <button class="tablinks active" @click="navigateToSavedVacancies">Saved Vacancies</button>
  ...
  methods: {
    ...
    async fetchJobs() {
      try {
        ...
        const response = await axios.get(`http://localhost:8085/vacancies/savers/${this.candidate.id}`, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        });
