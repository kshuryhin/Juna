<template>
  ...
  <main v-if="candidate">
    ...
    <button @click="refuseCandidate" class="refuse-button">Refuse</button>  </main>
  <main v-else>
    <p>Loading...</p>
  </main>
  <footer>
...
export default {
  ...
  methods: {
    async logout() {
     ...
     async refuseCandidate(){
      console.log(this.vacancy)
      this.vacancy.candidates = this.vacancy.candidates.filter(item => item.id !== this.candidate.id)
       await axios.put(`http://localhost:8085/vacancies/${this.vacancy.id}`, this.vacancy, {
         headers: { Authorization: localStorage.getItem("token") },
       })
           .then(response => {
             console.log(response.data);
             alert("Candidate refused!");
     ...
     async fetchVacancy(){
      ...
      await axios.get(`http://localhost:8085/vacancies/${vacancyId}`, {
        headers :{
    ...
    async fetchEmployerInfo(){
      await axios.get(`http://localhost:8085/employers/email`, {
        headers: { Authorization: localStorage.getItem("token") },
      }).then(response => {
...
.unsave-button:hover, .refuse-button:hover {
