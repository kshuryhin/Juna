<template>
  ...
  <div class="tabs">
    ...
    <button class="tablinks active" @click="navigateToMyVacancies">My Vacancies</button>
  </div>
  <main>
    ...
    <div class="content-container">
      <section id="jobs">
        <div class="jobs-container">
          <router-link to="/createVacancy" class="create-vacancy-btn">Create Vacancy</router-link>
          <ul id="job-listings">
...
export default {
  ...
  methods: {
    ...
    async fetchJobs() {
      try {
        const response = await axios.get(`http://localhost:8085/vacancies/employers/${this.employer.id}`, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        });
...
.create-vacancy-btn {
  ...
  border: 2px solid #168FF0; /* Add border */
}

.create-vacancy-btn:hover {
  background-color: #168FF0; /* Change to #168FF0 */
